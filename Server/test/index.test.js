...
describe("test de RUTAS", () => {
    describe("GET 'rickandmorty/character/:id", () => {
        ...
        it("Si hay un error responde con status: 500", async () => {
            const response = await request.get('/rickandmorty/character/999j');
    ...
    describe("GET '/rickandmorty/login", () => {
        ...
        it("Responde con un objeto con la propiedad access en true si la informacion del usuario es válida", async () => {
            const response = await request.get('/rickandmorty/login?email=santi@gmail.com&&password=123456')
            expect(response.body).toEqual(access)
        })
        it("Responde con un objeto con la propiedad access en false si la informacion del usuario no es válida", async () => {
            const response = await request.get('/rickandmorty/login?email=santi@mail.com&&password=12345fe')
            access.access = false;
            expect(response.body).toEqual(access)
        });
    });
    ...
    describe("DELETE /rickandmorty/fav/:id", () => {
        it("Si el ID solicitado no existe, debería retornar un arreglo con todos los favoritos", async () => {
            const response = await request.delete('/rickandmorty/fav/2gkdjk');
            expect(response.body.length).toBe(2);
        })
